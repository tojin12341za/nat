...
## 背景信息 {#section_kwj_gfz_ydb .section}
...
1.  为了方便调用API，本教程使用了一个用Python语言编写的Command Line工具。单击[此处](http://docs-aliyun.cn-hangzhou.oss.aliyun-inc.com/assets/attach/32327/cn_zh/1469718485900/api.py?spm=a2c4g.11186623.2.4.kwCDtJ&file=api.py)下载CLI工具。

    Linux环境下可以直接使用wget命令进行下载。

    ``` {#codeblock_9ok_uvw_zon}
...
## 步骤一 创建NAT网关 {#section_nrt_kge_yc3 .section}

1.  调用CreateNatGateway接口创建NAT网关。

    ``` {#codeblock_u62_w0o_bav}
...
## 步骤二 配置DNAT {#section_41c_dl1_ekt .section}
